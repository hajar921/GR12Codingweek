...
# Constants
MODEL_PATH = r"Training_Model\obesity_model.joblib"
...
def encode_user_input(user_input, data_dict):
    # Get the expected feature names and their order from the model
    expected_features = data_dict["feature_names"]
    
    # Create empty DataFrame with the correct column order
    input_df = pd.DataFrame(columns=expected_features, index=[0])
    
    # Fill in numerical features and categorical features
    for feature in expected_features:
        if feature in data_dict["numerical_features"]:
            input_df[feature] = user_input.get(feature, 0)  # Default to 0 if missing
        elif feature in data_dict["categorical_features"]:
            # Get the encoder for this feature
            encoder = data_dict["label_encoders"][feature]
            try:
                # Try to encode the value
                value = user_input.get(feature, encoder.classes_[0])  # Default to first class if missing
                encoded_value = encoder.transform([value])[0]
                input_df[feature] = encoded_value
            except ValueError:
                # If value not in encoder's classes, use the first class
                input_df[feature] = 0
        else:
            # For any other features, set to 0 as a default
            input_df[feature] = 0
